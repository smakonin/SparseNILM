...
class Accuracy:
    ...
    fscore           = lambda self, m = -1: round(2.0 * quotient(self.precision(m) * self.recall(m), self.precision(m) + self.recall(m)), 4)
    ...
    informedness     = lambda self, m = -1: round(self.tp_rate(m) + self.tn_rate(m) - 1.0, 4)
    markedness       = lambda self, m = -1: round(self.pos_predictive(m) + self.neg_predictive(m) - 1.0, 4)
    nde              = lambda self, m = -1: round(quotient(abs(self.est(m) - self.truth(m)), self.truth(m)), 4)
    rmse             = lambda self, m = -1: round(sqrt(quotient(1.0, mean(self.trials)) * mean(self.measure_diff_sq[m])) if m >= 0 else sum([self.rmse(i) for i in range(self.items)]), 4)
    ...
    kolter           = lambda self, m = -1: round(1.0 - quotient(self.diff(m), 2.0 * self.truth(m)), 4)
    ...
    m_fscore         = lambda self, m = -1: round(2.0 * quotient(self.m_precision(m) * self.m_recall(m), self.m_precision(m) + self.m_recall(m)), 4)
    ...
    fs_fscore        = lambda self, m = -1: round(2.0 * quotient(self.fs_precision(m) * self.fs_recall(m), self.fs_precision(m) + self.fs_recall(m)), 4)
    estacc           = lambda self, m = -1: round(1.0 - quotient(abs(self.est(m) - self.truth(m)), self.truth(m)), 4)
    ...
    def reset(self):
        ...
        self.count_atp = [[0.0 for s in range(self.folds)] for i in range(self.items)]
        self.count_itp = [[0.0 for s in range(self.folds)] for i in range(self.items)]
        self.count_tp = [[0.0 for s in range(self.folds)] for i in range(self.items)]
        self.count_tn = [[0.0 for s in range(self.folds)] for i in range(self.items)]
        self.count_fp = [[0.0 for s in range(self.folds)] for i in range(self.items)]
        self.count_fn = [[0.0 for s in range(self.folds)] for i in range(self.items)]
    ...
    def classification_result(self, fold, est, truth, states):
        ...
        for item in range(self.items):
            if est[item] > 0 and truth[item] > 0:
                ...
                self.count_tp[item][fold] += 1.0
            elif est[item] == 0 and truth[item] == 0:
                self.count_tn[item][fold] += 1.0
            elif est[item] > 0 and truth[item] == 0:
                self.count_fp[item][fold] += 1.0
            elif est[item] == 0 and truth[item] > 0:
                self.count_fn[item][fold] += 1.0
            else:
    ...
    def measurement_result(self, fold, est, truth):
        ...
        for item in range(self.items):
            ...
            self.measure_diff[item][fold] += abs(est[item] - truth[item])
            self.measure_diff_sq[item][fold] += (truth[item] - est[item])**2
            
            rho = 0.2
            if est[item] > 0.0 and truth[item] > 0.0:
                if float(abs(est[item] - truth[item])) / float(truth[item]) <= rho:
                    self.count_atp[item][fold] += 1.0
                else:
                    self.count_itp[item][fold] += 1.0

    def csv(self, test_id, labels, measure):
        """Get the results and CSV data."""
        
        hdr = 'Test ID,Item,Correct,Incorrect,TP,Inacc,APT,ITP,TN,FP,FN,Basic Acc,Precision,Recall,F-Score,M Precision,M Recall,M F-Score,FS Precision,FS Recall,FS F-Score,RMSE,NDE,Kolter,Est Acc,Estimated,Actual,Diff,Est of Total,Actual of Total'
        det = ''
        for i in range(-1, self.items):
            label = '*TL'
            if i > -1:
                label = labels[i]
            det += ','.join([str(v) for v in [test_id, label, self.correct(i), self.incorrect(i), self.tp(i), self.inacc(i), self.atp(i), self.itp(i), self.tn(i), self.fp(i), self.fn(i), self.accuracy(i), self.precision(i), self.recall(i), self.fscore(i), self.m_precision(i), self.m_recall(i), self.m_fscore(i), self.fs_precision(i), self.fs_recall(i), self.fs_fscore(i), self.rmse(i), self.nde(i), self.kolter(i), self.estacc(i), self.est(i), self.truth(i), self.diff(i), self.est_percent(i), self.truth_percent(i)]]) + '\n'
